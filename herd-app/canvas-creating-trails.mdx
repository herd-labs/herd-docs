...
There are three ways to add a primary step node right now:
...
Non-primary nodes are called "source" nodes, which provide the data for input values to other nodes. These can be easily added by clicking the "+ Add Source Node" button on the right side of each step.

<Frame>
  <img 
    src="/images/canvas-source-nodes.png" 
    alt="Canvas source nodes" 
    width="300"
    height="500"
  />
</Frame>

There are four types of source nodes:
...
Once you have added a node, you can click on the new node to pull up the edit panel. When a node is selected, any other nodes that have a purple outline are sources of data for this one, and any other nodes that have a blue outline are targets (rely on this one for data).
...
To find relevant existing code nodes created by others, you can use the search in the "+ Add Source Node" button/popup:
...
Any node (primary or source) in the trail can be turned into a "read" node by clicking the "read API" checkbox. This will tell the LLM/user that the data from that node can be useful for informing the next steps by the user. 
...
<Frame>
  <img 
    src="/images/canvas-customization.png" 
    alt="Canvas customization panel" 
    width="400"
    height="400"
  />
</Frame>
...
Here is an example of what the user sees when executing a trail on the Overlook:

<Frame>
  <img 
    src="/images/canvas-user-view.png" 
    alt="User view when executing a trail" 
    width="400"
    height="400"
  />
</Frame>

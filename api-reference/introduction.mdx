...
All API requests do not require an API key at the moment. There is a rate limit in place, if you require higher limits then please create an api key by going to `[trail apps](https://herd.eco/trail-apps) -> API key -> create key`. The API key header is `-H Herd-Trail-Api-Key: <api_key>`
